...
class Products extends Component {
        ...
        if (ans) {
            ...
            axios.delete(`/api/v1.0/product/delete/${id}`, { data: {token: token } }).then(res => {
                if (res.data.success === true) {
                    ...
                    this.setState({ total_item: total_items, items: total_items ,message: 'Producto eliminado' });
                }
            }).catch(error => {
                this.setState( { message: "No se pudo borrar el producto" } )
            });
        }
    }
            ...
            <div className={"row"}>
                <div className="card shadow mb-4 w-100">
                    <div className="card-header py-3">
                        <h5 className="m-0 font-weight-bold text-sr">Todos los productos</h5>
                        {
                            message !== '' ?
                            ...
                            <div className="row">
                                ...
                                <div className="col-sm-12 col-md-3">
                                    <button className="btn btn-primary btn-success mt-3"
                                            onClick={() => this.setState({section:"new"}) }>Crear producto</button>
                                </div>
                            </div>
    ...
    _renderNew = () => {
        const { artists, categories, errors } = this.state;
