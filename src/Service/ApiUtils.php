...
class ApiUtils
{
    ...
    public function getRequestParams(Request $request)
    {
        ...
        $request->query->has(self::AVAILABLE) ?
            $this->parameters[self::AVAILABLE] = intval($request->query->get(self::AVAILABLE)):null;
        ...
        $request->query->has(self::POST) ? $this->parameters[self::POST] = intval($request->query->get(self::POST)):null;
        $request->query->has(self::PRODUCT) ? $this->parameters[self::PRODUCT] = intval($request->query->get(self::PRODUCT)):null;
        $request->query->has(self::PURCHASE) ? $this->parameters[self::PURCHASE] = intval($request->query->get(self::PURCHASE)):null;
        ...
        $request->query->has(self::USER) ? $this->parameters[self::USER] = intval($request->query->get(self::USER)):null;
